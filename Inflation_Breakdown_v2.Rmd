...
CSO_HICP_CORE_INDEX<- t(data.frame(subset(subset(cso_get_data("CPM15"),Statistic=="EU HICP"),Commodity.Group =="HICP All items excluding energy and unprocessed food	")))%>% as.numeric()  %>% na.omit() %>% as.xts (order.by=Date_index)
...
Core_SERVICES_INDEX_DEC_2016 <- read_excel("C:/Users/flanagam/Documents/Core_SERVICES_INDEX_DEC_2016.xlsx") %>% .$Core_Services %>% as.numeric()
CORE_SERVICES_DATE<- SERVICES_DATE[194:(length(SERVICES_DATE)-6)]
...
DEC_BASE_GOODS[1:length(GOODS_INDEX_XTS["2017/2022"])]


overall_GOODS_index<-(TOTAL_CONTR_GOODS_sum)*DEC_BASE_GOODS[1:length(GOODS_INDEX_XTS["2017/2022"])] 
...
DEC_BASE_SERVICES[1:length(SERVICES_INDEX_XTS["2017/2022"])]


overall_service_index<-(TOTAL_CONTR_SER_sum)*DEC_BASE_SERVICES[1:length(TOTAL_CONTR_SER_sum["2017/2022"])]
...
#temporary fix to account for 2023 weights inclusion 
weight_CORE_SERVICES<-weight_CORE_SERVICES["/2022"]
#####################################################
...
#temporary fix to account for 2023 weights inclusion 
weight_monthly_Services_ONLY<-weight_monthly_Services_ONLY["/2022"]
weight_CORE_SERVICES_sum<-weight_CORE_SERVICES_sum["/2022"]
#####################################################
...
DEC_BASE_CORE_SERVICES[1:length(CORE_SERVICES_INDEX_XTS["2017/2022"])]


Core_service_index<-(TOTAL_CONTR_CORE_SER_sum)*DEC_BASE_CORE_SERVICES[1:length(TOTAL_CONTR_CORE_SER_sum["2017/2022"])]
...
#temporary fix below 
weight_monthly_Unprocessed_food_weight_ONLY<-weight_monthly_Unprocessed_food_weight_ONLY["/2022"]
...
weight_Unprocessed_food_weight_sum<-data.frame(NA, nrow=length(index(weight_monthly_Unprocessed_food_weight_ONLY["2017/2022"])))

for (i in 1:length(index(weight_monthly_Unprocessed_food_weight_ONLY["2017/2022"]))) {
...
Unprocessed_food_weight_TOTAL<-merge(OVERALL_UNPROCESSED_FOOD_XTS,index(UNPROCESSED_FOOD_INDEX["2017/2022"]))
...
CONTR_Unprocessed_food_sum_XTS<-as.xts(t(CONTR_Unprocessed_food_sum), order.by=index(Unprocessed_food_weight_TOTAL$OVERALL["2017/2022"]))
...
DEC_BASE_FOOD_NP[1:length(CONTR_Unprocessed_food_sum_XTS["2017/2022"])]


Unprocessed_food_weight_TOTAL_adj<-(Unprocessed_food_weight_TOTAL$OVERALL/1000)

Unprocessed_food_index<-(CONTR_Unprocessed_food_sum_XTS)*DEC_BASE_FOOD_NP[1:length(CONTR_Unprocessed_food_sum_XTS["2017/2022"])]
...
Processed_food_weight_ONLY_17<-Processed_food_weight_ONLY["2017/2022"]
...
CONTR_Processed_food_sum_XTS<-as.xts(t(CONTR_Processed_food_sum), order.by=index(Unprocessed_food_weight_TOTAL$OVERALL["2017/2022"]))
...
DEC_BASE_FOOD_P[1:length(CONTR_Unprocessed_food_sum_XTS["2017/2022"])]



Processed_food_index<-(CONTR_Processed_food_sum_XTS)*DEC_BASE_FOOD_P[1:length(CONTR_Processed_food_sum_XTS["2017/2022"])]
...
overall_good_services_index<-(Overall_XTS)*DEC_BASE_HICP_CONTRIBUTION[1:length(Overall_XTS["2017/2022"])]
...
OVERALL_INDEX_DEC_ONE_YEARLAG<-data.frame(lag.xts(OVERALL_HICP_XTS[paste0(year(SERVICES_GOODS_XTS_INDEX["2017/"]), "-12-01")],12)) %>% as.xts(index(SERVICES_GOODS_XTS_INDEX["2017/"]))
                                                                                                                                    OVERALL_INDEX_ONE_YEARLAG<-(lag.xts(OVERALL_HICP_XTS["2017/"],12))
                                                                                                                                    
OVERALL_INDEX_DEC_TWO_YEARLAG<-data.frame(lag.xts(OVERALL_HICP_XTS[paste0(year(SERVICES_GOODS_XTS_INDEX["2017/"]), "-12-01")],24))%>% as.xts(index(SERVICES_GOODS_XTS_INDEX["2017/"]))

IND_WEIGHT_ONE_YEARLAG<-(combo_weight["2017/"])
IND_INDEX_DEC_ONE_YEARLAG<-data.frame(lag.xts(SERVICES_GOODS_XTS_INDEX[paste0(year(SERVICES_GOODS_XTS_INDEX["2017/"]), "-12-01")],12)) %>%
as.xts(index(SERVICES_GOODS_XTS_INDEX["2017/"]))

IND_INDEX_ONE_YEARLAG<-(lag.xts(SERVICES_GOODS_XTS_INDEX,12))

IND_WEIGHT_TWO_YEARLAG<-lag.xts(combo_weight["2017/"],12)

IND_INDEX_DEC_TWO_YEARLAG<-data.frame(lag.xts(SERVICES_GOODS_XTS_INDEX[paste0(year(SERVICES_GOODS_XTS_INDEX["2017/"]), "-12-01")],24))%>%
as.xts(index(SERVICES_GOODS_XTS_INDEX["2017/"]))
...
EQ2  <-((SERVICES_GOODS_XTS_INDEX[,i]-IND_INDEX_DEC_ONE_YEARLAG[,i])/IND_INDEX_DEC_ONE_YEARLAG[,i])
...
energy_weight<-energy_weight["/2022"]
...
(ENERGY/growth(OVERALL_HICP_XTS,12))
...
Contribution_CHANGE<-CONTRIBUTION-lag(CONTRIBUTION,k=1)
...
WEIGHTS_GOODS_SERVICES_COMBO<-merge(SERVICES_GOODS_XTS_WEIGHT["2017/2022"],index(PROCESSED_FOOD_INDEX["2017/2022"]))
...
length<-round(length(x)*0.1)



exclude_top<-colnames((sort(x[200,],decreasing=TRUE))[1:9]) 
exclude_bottom<-colnames((sort(x[200,],decreasing=FALSE))[1:9])
...
DF<-data.frame(matrix(nrow = nrow(x), ncol = 9)) 
...
DF_decreased<-data.frame(matrix(nrow = nrow(x), ncol = 9)) 
...
DF_INDEX<-data.frame(matrix(nrow = nrow(x), ncol = ncol(x)-18)) 
...
for(i in 1:(nrow(DF_NAME_4_WEIGHT_SHORT["2017/"]))){
  DF_WEIGHT_SUM[i,]<-  sum(as.numeric(TOTAL_WEIGHT[i,paste(DF_NAME_4_WEIGHT_SHORT[i,1:nrow(DF_NAME_4_WEIGHT_SHORT)])]))  
...
write.xlsx(Trimmed_growth, file = 'growthrate.xlsx')
...
y1<-ggplot()+geom_line(data=graph_data["2017/"],aes(x=index(graph_data["2017/"]),y=graph_data$trim["2017/"], colour="DOF Trimmed Inflation"),size=1)+
  ...
  theme(legend.position=c(0.8,0.2),plot.caption = element_text(hjust=0))+
  ...
  labs(x="",
       ...
       caption="Date Source: CSO, Eurostat, DOF calculations",
       y="Percentage change (%)")+
...
betweeen_zero_two<-matrix(NA,ncol=ncol(growth_SERVICES_GOODS["2017/"]) ,nrow=nrow(growth_SERVICES_GOODS["2017/2022"]))
...
YTD_2023_annual<-ANNUAL_INFLATION["2023/"] %>% mean() 

FORECAST_KPI<-YTD_2023_annual-7.1
...
Contribution_highest_name<-sort(last(summary_contribution))[(ncol(summary_contribution))] %>% names()

Contribution_highest_number<-sort(last(summary_contribution))[(ncol(summary_contribution))] %>% round(1)
...
* As part of Budget 2023, the Department forecast Inflation growth of 7.1% in 2023. Year to date, annual inflation averaged `r YTD_2023_annual`% in `r CURRENT_DATE`. As a result inflation is currently running `r FORECAST_KPI_TEXT`. 
...
* At a sub-component level, `r names(sub_compon_max)` made the largest contribution to inflation, contributing `r sub_compon_max` percentage points to overall inflation growth. 

*Service prices `r growth_Services_TEXT` in `r CURRENT_DATE_COMPONENTS` on an annual basis, with strong contributions from 
...
```{r trim graph, echo=FALSE, warning=FALSE}
...
GG_heatmap<-GG_heatmap+labs(title="Heatmap of the highest contribution to annual inflation (excl.Energy)", caption="Date Source: CSO, Eurostat, DOF calculations",
